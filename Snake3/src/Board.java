...
public class Board extends JLabel{
	...
	public void paint (Graphics g) {
		...
		p = Snake.ptc(Snake.normalFood.getX(), Snake.normalFood.getY());
		g.fillRect(p.x, p.y, 32, 32);
		
	
		g.setColor(Color.PINK);
		p=Snake.ptc(Snake.runningFood.getX(), Snake.runningFood.getY());
		g.fillRect(p.x, p.y, 32, 32);
		...
		for(int i = 0; i < 16; i++) {
			for(int j = 0; j < 16; j++) {
				g.drawRect(i * 32 + Game.xCoordinate, j * 32 + Game.yCoordinate, 32, 32);
			}
		}
		
	
		g.setColor(Color.BLACK);
		g.drawRect(Game.xCoordinate, Game.yCoordinate, 512, 512);
		
	
		g.setColor(Color.MAGENTA);
		g.setFont(new Font("Arial", Font.BOLD, 22));
		g.drawString("Score:" + Snake.score, 10, 30);
		repaint();	
	}
}
